...
docker_setup_env() {
	# Get config
	declare -g DATADIR SOCKET WSREP_CLUSTER_ADDRESS WSREP
	DATADIR="$(mysql_get_config 'datadir' "$@")"
	SOCKET="$(mysql_get_config 'socket' "$@")"
	WSREP_CLUSTER_ADDRESS=$(mysql_get_config 'wsrep-cluster-address' "$@")
	WSREP=$(mysql_get_config 'wsrep-on' "$@")
	...
	file_env 'BOOTSTRAP'
	file_env 'CLUSTER_MEMBERS'
	file_env 'CLUSTER_NAME'
...
rebuild_config() {
	...
	HOST_IP_ADDRESS=$(awk 'END{print $1}' /etc/hosts)
	HOSTNAME=$(awk 'END{print $2}' /etc/hosts)
	SERVER_ID=`echo $HOST_IP_ADDRESS | cut -d . -f 4`

	cat << EOF >> /tmp/10-galera.cnf
[mysqld]
...
# Wsrep GTID mode attempts to keep GTIDs consistent for Galera Cluster write sets on all cluster nodes.  
wsrep_gtid_mode=ON
EOF
	cp /tmp/10-galera.cnf /etc/mysql/mariadb.conf.d/10-galera.cnf
	rm -rf /tmp/10-galera.cnf 
}

setup_galera() {
	if [ -z "$CLUSTER_MEMBERS" ]; then
		echo >&2 'error: You must specify at least one valid IP/hostname:PORT URI value for CLUSTER_MEMBERS in order to join the cluster'
		exit 1
	fi
	if [ -z "$CLUSTER_NAME" ]; then
		echo >&2 'error: You must specify a value for CLUSTER_NAME.'
		exit 1
	fi
	if [ ! -z "$BOOTSTRAP" ] && ! [ -f "/etc/mysql/mariadb.conf.d/10-galera.cnf" ]; then
		echo "Creating an empty configuration for initial setup"
		rebuild_config "empty"
	elif [ "$WSREP" = 'FALSE' ] || [ "$WSREP_CLUSTER_ADDRESS" = 'gcomm://' ]; then
		echo "Echo WSREP disabled. Recreating configuration"
		rebuild_config "full"
	elif [ "$WSREP_CLUSTER_ADDRESS" != 'gcomm://$CLUSTER_MEMBERS' ]; then
		echo "Topology change. Recreating configuration"
		rebuild_config "full"
	fi
}
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ ! -z "$BOOTSTRAP" ]; then
			BOOSTRAP_ARGS='--wsrep-new-cluster'
		fi
	fi
